import { isTerminalStatus } from "./lifecycle.ts";
...
export function applyTodoEvent(state: TodoState, event: TodoEvent): TodoState {
  ...
  if (event.type === "todo.created") {
    ...
    if (event.type === "todo.updated") todos[event.todoId] = updateTodo(todo, event.patch, event.at);
    ...
    if (event.type === "todo.reopened") todos[event.todoId] = updateTodo(todo, { status: event.targetStatus ?? "ready", completedAt: undefined, notes: event.reason ? [...todo.notes, `reopened: ${event.reason}`] : todo.notes }, event.at);
