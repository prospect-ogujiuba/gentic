...
## Post-cutover authority adoption

`/swe work adopt-post-cutover swe-production-rollout` is a one-purpose operator recovery surface for the reviewed rollout authority gap. It is available only in the exact blocked plan-review state with a settled infrastructure failure, pristine pending tasks, the current controlling v2 parent, no active child or todo, and the canonical 17-task graph. Print mode and the model-callable tool cannot invoke it.

Preparation pins the canonical rev41 workflow and all 17 task contract identities, requires the exact unstaged authority-file control change, and records two immutable ranges: rollout anchor → exact post-Session9 historical candidate `2c8fb31…` for tasks 1–16, then `2c8fb31…` → repair `HEAD` under a fixed authority-repair path allowlist. The retained approval report remains separately bound to its actual `08a434f…` candidate. A disposable worktree is created at repair `HEAD`. It installs dependencies from the committed lockfile into a real root directory, fingerprints the bounded dependency tree three times (after install, after review completion, and after the complete 38-check sequence; the representative project tree is guarded by a 30-second per-scan test budget) (dependency drift aborts the whole evidence bundle; checks share that measured sequence window), obtains fresh distinct plan/general/composed-concern reviews of one immutable cumulative snapshot, and reruns every exact verification command for tasks 1–16 through one-shot protected-executor grants bound to the current parent/session/runtime, task contract, snapshot, tree, command, and checkout. Grants reject forgery, replay, parent rotation, and source change; sequence-level fingerprints reject retained dependency drift. The command then displays the stable evidence hash and requires complete human authorization plus keyboard/RPC confirmation. One locked event retains the failed-run history and evidence hashes, marks only the exact prefix through `retire-v1-execution` historical, and leaves `cutover-release-qualification` pending. It never fabricates ordinary task reports, changes runtime selection, grants final acceptance, or authorizes release.
...
During managed execution the model parent may use only captured, provenance-stable read-only tools (including the pinned `git_snapshot` repository-status surface), read-only workflow status, and a pinned `intercom` surface limited to listing/status, pending-thread inspection, and an unrouted attachment-free reply to the active inbound thread. Tool trust compares captured provenance, source bytes, and public metadata because Pi returns fresh metadata wrappers from each `getAllTools()` call; wrapper object identity is not a tool identity. It cannot send or ask new peers, select another target/cwd, create a pane, or attach content. Direct `edit`, `write`, general `bash`, alternate shells, `ctx_execute`, MCP execution, `interactive_shell`, batch/nested wrappers, and unknown or dynamically replaced tools fail closed. Protected `bash` is enabled only in the verification phase for the exact planned executable/arguments and cwd. Echo-style fabricated checks are invalid contracts.
