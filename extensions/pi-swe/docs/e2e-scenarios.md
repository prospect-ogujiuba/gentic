...
2. Run `/swe work start <topic>`. Confirm the persisted runner is owner-bound, `guided`, `until: context`, has unlimited turns/time unless configured, and retains the bounded retry policy. Confirm exactly one expanded `/skill:swe-implement` prompt names the current plan/contract path/hash and dispatch token.
...
4. Run `/swe work pause <topic>`, then `/swe work resume <topic>` from the owning session. Confirm pause wins before continuation and resume never repeats a sent dispatch token. Pause again, open a fresh Pi session, and run `resume`; confirm it atomically takes ownership with a fresh run identity, clears any old pending dispatch, and prevents the prior owner from mutating the replacement. Run `/swe work stop <topic>` from the current owner and confirm later settled events queue nothing.
5. Exercise the default context-scoped run across contract completion and confirm it advances only to dependency-ready work, then pauses before another dispatch when exact `pi-context` pressure becomes critical. Start a separate run with `--until initiative --max-turns 3 --max-minutes 10` and confirm explicit numeric limits and finalization remain checkpoint-gated.
6. In separate disposable runs, verify ambiguous topic selection, stale plan/hash, missing verifier/capability, conflicting changes, unsafe operation, external side effect, exhausted retry/turn/time budget, malformed checkpoint, and raced ownership transfer all stop or pause with diagnostics and no follow-up.
