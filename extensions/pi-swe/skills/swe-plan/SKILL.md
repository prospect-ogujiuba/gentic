...
2. Use `.model-artifacts/initiatives/<topic>/specs/manifest.json` as the mutable machine-readable initiative index and `.model-artifacts/initiatives/<topic>/specs/YYYY-MM-DD_HHMM-initiative-spec-rN.md` for immutable specification revisions.
3. Use `.model-artifacts/initiatives/<topic>/plans/YYYY-MM-DD_HHMM-plan-index-rN.md` for immutable plan-index revisions.
4. Set `activePlan.contractRoot` to `.model-artifacts/initiatives/<topic>/plans/revisions/rN`, put phase and subphase contracts below its `phases/` directory, and maintain `<activePlan.contractRoot>/contracts.json` as the machine-readable contract index.
5. Put specialist findings below `.model-artifacts/initiatives/<topic>/findings/` and plan reviews below `.model-artifacts/initiatives/<topic>/reports/`; each artifact links the exact spec and plan revision it assessed.
...
5. **Plan review** — invoke `swe-review` in plan-review mode, set the reviewing state, and write the plan review under `.model-artifacts/initiatives/<topic>/reports/`. Do not use implementation-review findings as plan approval.
