...
- **State/config files it reads/writes:** uses `PI_GATE_CONFIG` as the explicit global read/write target; otherwise uses `<PI_CODING_AGENT_DIR>/pi-gate/pi-gate.json` when that Pi override is set, falling back to `~/.pi/pi-gate/pi-gate.json`. Project config lives at `<project>/<CONFIG_DIR_NAME>/pi-gate/pi-gate.json` (normally `<project>/.pi/pi-gate/pi-gate.json`). It writes audit JSONL to `.pi/pi-gate/pi-gate-audit.jsonl` by default and keeps session-only decisions and stats in memory.
...
- **Known boundaries/non-goals:** this is conservative classification, not a complete shell parser or sandbox. Configured `permissions` are explicit globs; remembered commands are stored separately in `literalPermissions` and match exactly. Native prompt choices use arrow navigation and confirmation rather than letter shortcuts. Invalid reloads retain the last-known-good policy.
