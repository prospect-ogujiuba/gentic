import { existsSync, lstatSync, mkdirSync, readFileSync, realpathSync, renameSync, rmSync, rmdirSync, writeFileSync } from "node:fs";
...
export function applyScaffold(kind: ScaffoldKind, name: string, variant?: ScaffoldVariant, options?: ScaffoldOptions): ScaffoldApplyResult {
  ...
  for (const file of preview.files) if (existsSync(assertSafeTargetAncestors(preview, file.target))) throw new Error(`Refusing to overwrite existing scaffold target: ${file.target}`);
