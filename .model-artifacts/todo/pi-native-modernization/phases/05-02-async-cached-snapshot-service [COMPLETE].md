...
- DSA finding: `.model-artifacts/findings/pi-native-modernization/2026-08-21_2048-async-snapshot-dsa-decision.md`.
- Service/collector tests: `test/pi-hud-snapshot-service.test.ts` cover coalescing, debounce, freshness, last-good errors, unavailable state, generation reset, cancellation, disposal, slow process responsiveness, timeout, command failure, non-repository, and oversized output.
- `npm run typecheck`, `npm run check`, focused pi-hud tests, and the full 280-test suite pass.
- Static scan confirms no synchronous child-process call remains under `extensions/pi-hud/`.
