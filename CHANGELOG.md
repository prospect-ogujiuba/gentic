...
- Version-stamped build-time Pi capability fixture and source/manifest Gentic inventory.
...
- Catalog UX is consolidated into runtime-backed `/catalog [status|search <term>]` and the compact `gentic_catalog` tool; the overlapping `/gentic` command and `gentic_status` tool are removed.
